/**
 * Эталон для xi.tutor/modules.calls/src/useTanstackCallsNavigation.ts
 *
 * Маршруты должны совпадать с file-based routing xi.tutor.
 * При расхождении — скорректируйте `to:` / `params` / `search`.
 */
  ...
  const navigation = useMemo<CallsNavigationT>(
    () => ({
      ...
      navigateToClassroomOverview: (classroomId, options) => {
        ...
        if (options?.backgroundCall !== false) {
          search.call = classroomId;
        }
        navigate({
          to: '/classrooms/$classroomId',
