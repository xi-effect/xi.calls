...
const NavigationButton = ({ onClick, disabled, orientation, direction }: NavigationButtonProps) => {
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className="disabled:fill-icon-primary fill-icon-primary z-10 bg-transparent p-0 text-center hover:opacity-100 disabled:cursor-not-allowed"
    >
      <div className="bg-background-surface flex items-center justify-center rounded-full p-2">
...
export const Carousel = ({
  ...
  <div className="mx-auto h-full w-full">
    <div className="relative h-full overflow-hidden">
      ...
      <div
        className={`${orientation === 'vertical' ? 'my-14 flex min-h-0 flex-col' : 'mx-14'} relative z-0 flex h-full min-h-0 w-full touch-pan-x snap-x snap-mandatory gap-5 overflow-hidden scroll-smooth`}
      >
        {children}
      </div>
    </div>
